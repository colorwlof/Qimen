...
def parse_args():
    parser = argparse.ArgumentParser(description='無參數則以當前時間拆補法起局')
    parser.add_argument('--y', type=int, default=datetime.now().year,
                        help='年')
    parser.add_argument('--m', type=int, default=datetime.now().month,
                        help='月')
    parser.add_argument('--d', type=int, default=datetime.now().day,
                        help='日')
    parser.add_argument('--h', type=int, default=datetime.now().hour,
                        help='时')
    parser.add_argument('--f', type=int, default=datetime.now().minute,
                        help='分')
    parser.add_argument('--n', type=int, default=1,
                        help='1:拆補 2:置閏,默認1')
    parser.add_argument('--r', type=int, default=-1,
                        help='报数开局')
    parser.add_argument('--o', type=str, default="paipan.txt",
                        help='报数开局')
...
if __name__ == '__main__':
    ...
    # print(Qimen(2024,2,2,4,15).pan_earth(2))
    # print(Qimen(2024, 2, 2, 4, 15).pan(1))
    ...
    def run():
        ...
        if args.r != -1:
            random.seed(args.r)
            args.y = int(random.randint(2000, 3000))
            args.m = int(random.randint(1, 12))
            args.d = int(random.randint(1, 30))
            args.h = int(random.randint(0, 23))
            args.f = int(random.randint(0, 60))
        result = Qimen(args.y, args.m, args.d, args.h, args.f).pan(args.n)
        print(result)
        with open(args.o, 'w', encoding='utf-8') as f:
            f.write(f"{result}")
        return result
        # print(Qimen(2025,4,1,17,30).pan(1))
