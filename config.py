...
gong_wuxing = dict(zip(clockwise_eightgua,"水土木木火土金金"))
gong_dizhi = {'巽': ['辰', '巳'], '離': ['午'], '坤': ['未', '申'], '兌': ['酉'], '乾': ['戌', '亥'],
              '坎': ['子'], '艮': ['丑', '寅'], '震': ['卯'], '中': ['']}
#%% 基本功能函數
...
def find_shier_luck(gan):
    cs = re.findall('..',"長生沐浴冠帶臨官帝旺")
    fullcs=lambda y: dict(zip(y, cs + list("衰病死墓絕胎養")))
    nlist = list(map(lambda i:new_list(di_zhi, i),list("亥寅寅巳申")))
    nnlist = list(map(lambda i:new_list_r(di_zhi, i), list("午酉酉子卯")))
    cslist2 = list(map(fullcs, nlist))
    cslist = list(map(fullcs, nnlist))
...
def pan_star(year, month, day, hour, minute, option):
    ...
    star_r = list("蓬任沖輔英芮柱心")
    starting_star = zhifunzhishi.get("值符星宮")[0].replace( "禽","芮")
...
def pan_star_minute(year, month, day, hour, minute, option):
    ...
    qimen_ke = qimen_ju_name_ke(year, month, day, hour, minute)
    starting_star = zhifu_n_zhishi.get("值符星宮")[0].replace( "禽","芮")
...
def pan_god(year, month, day, hour, minute, option):
    ...
    if starting_gong == "中":
        ...
        gong_reorder = new_list(rotate, starting_gong)
    return dict(zip(gong_reorder,{"陽":list("符蛇陰合虎玄地天"),
